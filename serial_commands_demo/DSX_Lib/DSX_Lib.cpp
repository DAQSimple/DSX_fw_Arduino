...
void exec_command(DSXpacket_t packet) {
	...
	else if (strcmp(packet.ID, "hamburger") == 0) {
		Serial.println("MMMMmmmmmm");
	}
	else {
 ...
 *
 * @note   	Use pin 4 on the arduino uno
 *
 * @param  	None
 *
 * @retval 	None
 */
 void readEncoder() {
	if(digitalRead(encoderChA) == LOW) 	// since this pin is pulled up HIGH by default
		duration=pulseIn(encoderChA,LOW);	//in microseconds
	if(duration != 0)
